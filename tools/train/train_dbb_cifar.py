...
from net.dbb_series.resnet import create_Res18
...
def main():
    setup_env()
    # Networks
    # resnet_101 = resnet101().cuda()
    # ckpt = torch.load('weights/resnet101.pth')
    # resnet_101.load_state_dict(ckpt)
    
    # # ckpt = torch.load(cfg.DECO.PATH)
    # student_net = RepVGG_A1(num_classes=100).cuda()
    # student_net._train()
    # # student_net.set_attach_rate(1.)
    # student_net.turn_only310(True)
    # # student_net.load_state_dict(ckpt['model_state'])
    
    student_net = create_Res18(cfg.LOADER.NUM_CLASSES, cfg.POST.DBB_TYPE, False).cuda()
    # student_net.turn_only_raw(True)
    # student_net.set_attach_rate(1.)
    ...
    # criterion = KD_Loss(alpha=0., temperature=cfg.DECO.TEMPERATURE)
    ...
    for cur_epoch in range(cfg.OPTIM.MAX_EPOCH):
        train_epoch(cur_epoch, student_net, train_loader, train_meter, optimizer, scheduler, criterion)
...
def train_epoch(cur_epoch, student_net, train_loader, train_meter, optimizer, scheduler, criterion):
    # teacher_net.eval()
    student_net.train()
    ...
    for cur_iter, (inputs, labels) in enumerate(train_loader):
        ...
        # loss = soft_criterion(student_preds, teacher_preds, labels)
